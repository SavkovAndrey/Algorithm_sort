...
//------------------------- СОРТИРОВКА: КУЧЕЙ (формирование кучи)
void Heap(int* mass, int size, int root)                         // в функцию передается "рассматриваемый" корень (индекс)
{
	int largest = root;                                          // считаем наибольшим КОРЕНЬ
	int l = 2 * root + 1;                                        // индекс эл. слева от корня
	int r = 2 * root + 2;                                        // индекс эл. справа от корня

	if (l < size && mass[l] > mass[largest])            // Есть ли ЛЕВЫЙ эл. в массиве!? и не больше ли он КОРНЯ?
		largest = l;                                    // если да - то он корень!

	if (r < size && mass[r] > mass[largest])            // Есть ли ПРАВЫЙ эл. в массиве!? и не больше ли он корня?
		largest = r;                                    // если да - он корень!

	if (largest != root)                                // Если индекс корня изменился верхними if, меняем и сам корень
	{
		swap(mass[root], mass[largest]);
		Heap(mass, size, largest);                      // Если корень сменился, то нужно проверять дерево снова - от
	}                                                   // корня, который "сполз" вниз...
}

//------------------------- СОРТИРОВКА: КУЧЕЙ
void HeapSort(int* mass, int size)
{
	for (int i = size / 2 - 1; i >= 0; i--)            // n/2 - 1 , формула для определения первого эл. который сортируем
		Heap(mass, size, i);                           // ПРИМЕР: при size 5, сначал i = 1, потом 0... это корни 
	                                                   // которые рассматриваются... Если size больше рассмотрятся все,
	                                                   // что нужны
	for (int i = size - 1; i >= 0; i--)                
	{                                                  // кидаем корень дерева к конец массива и уменьшаем size - 1,
		swap(mass[0], mass[i]);                        // и более не трогаем этот эл. - он отсортирован...
		Heap(mass, i, 0);                              // занаво сортируем дерево, уже с размером на 1 меньше... 
	}
}